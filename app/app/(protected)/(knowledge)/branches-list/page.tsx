...
const Page: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, "branches_list"));
    ...
    return (
        <div className="flex flex-col gap-4 p-6 bg-background">
            {processFlows.map((flow) => (
                <Card key={flow.id} className="self-end max-w-xs p-4 rounded-lg shadow-md mx-auto border border-border-hover bg-background-hover">
                    <div className="flex items-center gap-3">
                        <Avatar icon={getFileIcon(flow.fileType)} isBordered />
                    ...
                    <div className="mt-3">
                        ...
                        ) : flow.fileType.includes("excel") || flow.fileType.includes("spreadsheet") ? (
                            <div className="p-2 bg-green-100 rounded text-green-700">Excel File - Open Online</div>
                        ) : flow.fileType.includes("ppt") ? (
                            <div className="p-2 bg-orange-100 rounded text-orange-700">PowerPoint - Open Online</div>
                        ) : null}
                    </div>
                    ...
                    <div className="flex gap-2 mt-3">
                        <Button onPress={() => handleDownload(flow.fileUrl)} className="bg-primary hover:bg-primary-hover text-white w-1/2">
                            Download
                        </Button>
                        <Button onPress={() => handleOpenOnline(flow.fileUrl, flow.fileType)} className="bg-secondary hover:bg-secondary-hover text-white w-1/2">
                            Open Online
                        </Button>
                    </div>
                </Card>
