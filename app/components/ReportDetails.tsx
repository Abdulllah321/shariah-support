...
const ReportDetails: React.FC<ReportDetailsProps> = ({ id }) => {
  ...
  const action =
    actionParam === "branch-shariah" ? "branch-review" : actionParam;
...
const DetailItem: React.FC<DetailItemProps> = ({ label, value, icon }) => {
  ...
  return (
    <Card
      className={`border-l-4 border-secondary dark:border-accent-dark shadow-md dark:bg-gray-800 dark:text-gray-300 ${
        isArray ? "col-span-2" : ""
      }`}
    >
      <CardHeader>
        {icon}
        <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-200 ml-2">
          {label}
        </h3>
      </CardHeader>
          ...
          {isArray ? (
            <ul className="mt-2 space-y-2">
              {value.map((q, i) => (
                ...
                >
                  <h2 content={`#${i + 1}`}>Question #{i + 1}</h2>
            ...
            <p className="text-lg text-gray-800 font-medium dark:text-gray-100">
              {value}
            </p>
          )}
        </div>
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-4">
      {[...Array(4)].map((_, index) => (
        <div
          key={index}
          className="p-4 rounded-lg shadow-md animate-pulse bg-gray-200 dark:bg-gray-700"
        >
