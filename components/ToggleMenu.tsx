...
export default function ToggleMenu({ user, handleLogout }: ToggleMenuProps) {
    ...
    const menuSections = useMemo(() => getMenuSections(handleLogout), [handleLogout]);
            ...
            <DropdownMenu
                ...
                topContent={
                    <div className="pt-3 pb-1">
                        <User name={user?.username || "User"} description={`#${user?.employeeId}`} />
            ...
            >
                {menuSections.map(({ title, items }:MenuSection, index:number) => (
                    <DropdownSection key={index} title={title!} showDivider={!!title}>
                        {items.map(({ key, label, href, icon: Icon, action, className }: MenuItem) => {
                            ...
                            return (
                                <DropdownItem
                                    ...
                                    href={href}
                                    onPress={action}
                                    className={className || (isActive ? "bg-primary shadow-foreground shadow-sm text-white" : "bg-transparent")}
