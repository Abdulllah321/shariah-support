...
export default function ProcessFlowUploader() {
    ...
    const fetchProcessFlows = async () => {
        const snapshot = await getDocs(collection(db, "branches_list"));
    ...
    const handleUpload = async () => {
        ...
        formData.append("folder", "branches_list"); // 🔹 یہاں اپنا فولڈر کا نام دیں
        ...
        try {
            ...
            if (editingId) {
                await updateDoc(doc(db, "branches_list", editingId), { description, fileUrl, fileType });
            } else {
                await addDoc(collection(db, "branches_list"), { description, fileUrl, fileType, createdAt: new Date() });
            }
    ...
    const handleDelete = async (id, fileUrl) => {
        if (confirm("Are you sure you want to delete this item?")) {
            await deleteDoc(doc(db, "branches_list", id));
            ...
            <div className={'grid md:grid-cols-3 grid-cols-1 gap-10'}>
                <div>
                    ...
                    <button
                        ...
                        disabled={uploading}
                    >
                        {uploading ? "Uploading..." : <><FiUpload size={20} /> {editingId ? "Update" : "Upload"} Branch List</>}
                    </button>
                </div>
